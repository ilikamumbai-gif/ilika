  ...
  return (
    <div className="w-full">
      <Link to={`/product/${slug}`} state={{ id: productId }} className="group block">
        <article className="overflow-hidden  border border-[#b34140] bg-white transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(0,0,0,0.08)] ">

          {/* -- Image area -- */}
          <div className="relative aspect-square overflow-hidden bg-white">
            {!product.inStock && (
              <span className="absolute right-2 top-2 z-20 inline-flex items-center justify-center bg-[rgb(43,42,41)] px-2 py-1 text-[9px] font-bold text-white shadow-sm sm:right-3 sm:top-3 sm:text-[10px]">
                Out Of Stock
              </span>
            )}
            {productTag && (
              <span
                className={`absolute ${product.inStock ? "right-2 top-2 sm:right-3 sm:top-3" : "right-2 top-8 sm:right-3 sm:top-10"} z-20 inline-flex max-w-[55%] items-center justify-center truncate px-2 py-1 text-[9px] font-bold text-white shadow-sm sm:text-[10px] ${buttonBg}`}
                title={productTag}
              >
                {productTag}
              </span>
            ...
            <div className="absolute bottom-2 left-2 right-2 z-20 flex items-end gap-2 sm:bottom-3 sm:left-3 sm:right-3 sm:gap-2.5">
              <span className="inline-flex min-h-[28px] min-w-[56px] items-center justify-center bg-[rgba(255,255,255,0.82)] px-2 py-1 text-[8px] font-bold uppercase leading-[1.05] tracking-[0.04em] text-[#111] backdrop-blur-md shadow-[0_4px_10px_rgba(0,0,0,0.08)] sm:min-h-[30px] sm:min-w-[62px] sm:text-[10px]">
                {calculatedDiscount ? `${calculatedDiscount}% Off` : "Off"}
              </span>
              {couponOfferText && (
              <div className="max-w-[calc(100%-64px)] sm:max-w-[calc(100%-72px)]">
                <div className="relative overflow-hidden border border-[#f3c7bf] bg-[linear-gradient(135deg,_#fff6f2_0%,_#ffe9df_100%)] px-3 py-2 shadow-[0_8px_18px_rgba(179,65,64,0.08)] sm:px-3.5 sm:py-2.5">
          ...
          {/* -- Info area -- */}
          <div className="px-2.5 pt-2 pb-2 sm:px-4 sm:pt-3 sm:pb-3">

            {/* Product name */}
            <h4
              className="mb-2 h-[34px] w-full overflow-hidden text-left text-[13px] font-semibold leading-[1.35] tracking-[0.01em] text-[#1e1e1e] line-clamp-2 sm:mb-2.5 sm:h-[42px] sm:text-[15px] sm:leading-[1.35]"
              title={product.name}
            >
              {product.name}
            </h4>
            ...
            <div className="flex flex-col gap-2.5 sm:flex-row sm:items-end sm:justify-between sm:gap-3">
              ...
              <button
                ...
                className={`inline-flex h-8 w-full shrink-0 items-center justify-center gap-1.5 border border-[#2f2f2f] px-3 text-[10px] font-semibold uppercase tracking-[0.04em] transition-all duration-150 hover:scale-105 active:scale-95 sm:h-9 sm:w-auto sm:gap-2 sm:px-4 sm:text-[11px] ${product.inStock ? `${buttonBg} ${buttonText}` : "bg-[rgb(43,42,41)] text-white hover:opacity-95"}`}
              >
                {product.inStock ? (
                  <span>Add +</span>
                ) : (
                  <>
                    <FiBell className="text-[13px] sm:text-[18px]" />
                    <span>Notify</span>
                  </>
                )}
              </button>
