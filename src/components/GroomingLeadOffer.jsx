...
import { X, CheckCircle2, Gift, MessageCircleMore } from "lucide-react";
...
const COUPON_MESSAGE = "Offer unlocked: ₹500+ OFF on selected premium grooming appliances.";

const ELIGIBLE_PRODUCT_MATCHERS = [
  { label: "Ilika IPL Laser Hair Removal Device", keywords: ["ipl", "laser hair removal"] },
  { label: "Ilika High-Speed Leafless Hair Dryer", keywords: ["leafless hair dryer", "bldc hair dryer"] },
  { label: "Ilika Beauty Bubble Pro Blackhead Remover", keywords: ["beauty bubble", "blackhead remover"] },
  { label: "Ilika Hot & Cold Blackhead Remover", keywords: ["hot & cold blackhead remover", "hot cold blackhead remover"] },
  { label: "Ilika Ultrasonic Facial Cleansing Brush", keywords: ["ultrasonic facial cleansing brush", "facial cleansing brush"] },
  { label: "Ilika Automatic Voice-Guided Face Mask Maker", keywords: ["voice version face mask maker", "voice-guided", "mask maker machine"] },
];
...
const matchesEligibleProduct = (product = {}, keywords = []) => {
  const haystack = [
    product?.name,
    product?.productUrl,
    product?.slug,
...
const GroomingLeadOffer = ({
  ...
  const eligibleProducts = useMemo(() => {
    ...
    ELIGIBLE_PRODUCT_MATCHERS.forEach((matcher) => {
      const match = activeProducts.find(
        (product) =>
          product?.isActive !== false &&
          !seenIds.has(String(product?.id || product?._id || "")) &&
          matchesEligibleProduct(product, matcher.keywords)
      );

      if (match) {
        const id = String(match?.id || match?._id || matcher.label);
        seenIds.add(id);
        selected.push(match);
      }
    });
    ...
    try {
      ...
      const response = await fetch(getApiUrl("/api/leads"), {
        ...
        body: JSON.stringify({
          ...
          pageUrl: typeof window !== "undefined" ? window.location.href : `${location.pathname}${location.search}${location.hash}`,
        }),
      });
  ...
  return (
    <>
      ...
      {isUnlocked ? (
        <section
          ref={sectionRef}
          className="mx-auto mt-8 w-full max-w-7xl rounded-[30px] border border-[#efdce4] bg-[linear-gradient(180deg,#fff8fb_0%,#ffffff_100%)] px-4 py-6 shadow-[0_20px_50px_rgba(209,90,143,0.08)] sm:px-6 sm:py-8"
        >
          <div className="flex flex-col gap-4 border-b border-[#f3dfe7] pb-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h3 className="mt-3 text-2xl font-semibold text-[#111111] sm:text-[30px]">
                Premium Grooming Appliance Offer
              </h3>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#5f5660]">
                {COUPON_MESSAGE} We’ll send the WhatsApp offer details to your submitted number shortly.
              </p>
            </div>

            <div className="rounded-2xl border border-[#f1d4df] bg-white px-4 py-3 text-sm text-[#5f5660] shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#b24074]">Offer Value</p>
              <p className="mt-1 text-lg font-semibold text-[#111111]">{COUPON_VALUE} OFF</p>
            </div>
          </div>

          <div className="mt-6 rounded-2xl border border-dashed border-[#e7c7d5] bg-[#fff7fa] px-4 py-4 text-sm text-[#5f5660]">
            Offer message:
            <span className="ml-2 font-semibold text-[#111111]">
              WhatsApp-exclusive appliance savings unlocked for premium Ilika grooming tools.
            </span>
          </div>
          ...
          {eligibleProducts.length ? (
            ...
            <div className="mt-6 rounded-2xl border border-[#f1d4df] bg-white px-4 py-5 text-sm text-[#5f5660]">
              Eligible grooming appliance products will appear here as soon as they are available in your product data.
              <Link to="/grooming" className="ml-2 font-semibold text-[#b24074] underline underline-offset-4">
                Browse grooming appliances
              </Link>
            </div>
          )}
        </section>
