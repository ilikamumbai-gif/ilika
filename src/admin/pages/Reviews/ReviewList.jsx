...
const ReviewList = () => {
  ...
  const deleteReview = async (productId, reviewIndex, name, productName) => {
    ...
    await fetch(`${API}/api/reviews/${productId}/${reviewIndex}`, { method: "DELETE" });
    await logActivity(`Deleted review by ${name} on ${productName}`);
    fetchReviews();
  };

  const filtered = reviews.filter(r =>
    (!search || r.productName?.toLowerCase().includes(search.toLowerCase()) || r.name?.toLowerCase().includes(search.toLowerCase())) &&
    (!ratingFilter || String(r.rating) === ratingFilter)
  );
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #F0F0F0" }}>
                    {["Product", "Reviewer", "Rating", "Comment", "Actions"].map(h => (
                ...
                <tbody>
                  {filtered.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50/70 transition-colors" style={{ borderBottom: "1px solid #F5F5F5" }}>
                      <td className="px-5 py-4 font-semibold text-gray-800 max-w-[160px] truncate">{row.productName}</td>
                      <td className="px-5 py-4 text-gray-600">{row.name || "—"}</td>
                      <td className="px-5 py-4"><StarRating rating={row.rating || 0} /></td>
                      <td className="px-5 py-4 text-gray-500 max-w-xs">
                        <p className="truncate">{row.comment ? `${row.comment.slice(0, 60)}${row.comment.length > 60 ? "…" : ""}` : "—"}</p>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => deleteReview(row.productId, row.reviewIndex, row.name, row.productName)}
                ...
                <div key={row.id} className="p-4 space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      <button onClick={() => navigate(`/admin/reviews/${row.productId}/${row.reviewIndex}`)}
                        className="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 text-blue-600">
                        <Eye size={14} />
                      </button>
                      <button onClick={() => deleteReview(row.productId, row.reviewIndex, row.name, row.productName)}
