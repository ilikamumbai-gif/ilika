...
const rules = {
  skinType: {
    ...
    sensitive: [
      "Gentle Cleanser | Prevent Signs of Aging | 100ML",
      "Tea Tree & Avocado Toner, Peeling Solution | Refreshes & Revitalize | 100ML",
      "Ceramide Gel Moisturizer | Repairs Sun Damage | 50g",
      "Hydra Gel Face Moisturizer | For Dry & Dehydrated Skin 50g",
      "Belgium Rose Toner | Soothing & Refreshing | 100 ML",
    ],
...
const optionProductHints = {
  concerns: {
    acne: "Tea Tree & Avocado Toner, Foaming Face Wash",
    dehydrated: "Belgium Rose Toner, Hyaluronic Acid Serum",
    antiaging: "Retinol Anti-aging Facial Oil, Collagen Serum",
    enlargedPores: "Tea Tree & Avocado Toner, Peeling Solution",
    darkCircles: "Under Eye Serum | Fast-Absorbing Formula | 30 ML",
    unevenTexture: "Peach & Jojoba Toner, Peeling Solution",
    darkSpots: "Brown Spot Corrector, Dazzling Face Serum",
    scars: "Anti Scar Facial Oil",
  },
  skinType: {
    oily: "Tea Tree & Avocado Toner, Hydra Gel Moisturizer",
    dry: "Belgium Rose Toner, Ceramide Gel Moisturizer",
    combination: "Peach & Jojoba Toner, Hydra Gel Moisturizer",
    normal: "Blooming Vine Toner, Ceramide Gel Moisturizer",
    sensitive: "Tea Tree & Avocado Toner, Peeling Solution, Ceramide Gel Moisturizer",
  },
  goal: {
    skinLightening: "Dazzling Face Serum, Brown Spot Corrector",
    antiAging: "Gentle Cleanser, Collagen Serum",
  },
  age: {
    "15-20": "Foaming Face Wash, Hydra Gel Moisturizer",
    "21-30": "Hyaluronic Acid Serum, Dazzling Fair Serum",
    "31-45": "Retinol Anti-aging Facial Oil, Collagen Serum",
    "46-55": "Retinol Anti-aging Facial Oil, Gentle Cleanser",
    above55: "24k Gold Beauty Oil, Ceramide Gel Moisturizer",
  },
};
...
const KnowSkinType = () => {
  ...
  const findProduct = (ruleName = "") => {
    ...
    return activeProducts.find((p) => normalizeName(p?.name || "").includes(key) || key.includes(normalizeName(p?.name || ""))) || null;
  };
    ...
    const pickMandatory = (type) => {
      if (type === "oil" && (user.age === "31-45" || user.age === "46-55")) {
        ...
        const forcedFallback = activeProducts.find(
          (p) => normalizeName(p?.name || "") === normalizeName(forcedOilName) || normalizeName(p?.name || "").includes(normalizeName(forcedOilName))
        );
        if (forcedFallback) {
      ...
      const fallback = activeProducts.find((p) => routineType(p?.name) === type);
      return fallback ? { product: fallback.name, score: 1, matchedProduct: fallback } : null;
    };
      ...
      <main className="min-h-screen bg-[#f6f4f4] py-8">
        <section className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="bg-white rounded-3xl border border-[#e7dfdf] p-5 sm:p-8">
              ...
              <div className="flex items-center justify-between gap-3">
                {stepTitles.map((label, i) => (
                  <div key={label} className="flex-1 min-w-0">
                    <div className={`h-1 rounded-full ${i < progress ? "bg-[#0ea5b7]" : "bg-gray-300"}`} />
            ...
            <h1 className="text-2xl sm:text-3xl font-semibold text-[#0b8ea3]">Welcome To ILIKA</h1>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {options.ages.map(([key, label]) => (
                        <button
                          ...
                          className={`text-left rounded-2xl border px-4 py-3 transition ${form.age === key ? "border-[#0ea5b7] bg-[#eaf9fc]" : "border-[#e5e7eb] bg-[#f7f7f8]"}`}
                        >
                          {label}
                        </button>
                      ))}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {options.concerns.map(([key, label]) => (
                        <button
                          ...
                          className={`text-left rounded-2xl border px-4 py-3 transition ${form.concerns.includes(key) ? "border-[#0ea5b7] bg-[#eaf9fc]" : "border-[#e5e7eb] bg-[#f7f7f8]"}`}
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {options.goals.map(([key, label]) => (
                        <button
                          ...
                          className={`text-left rounded-2xl border px-4 py-3 transition ${form.goal === key ? "border-[#0ea5b7] bg-[#eaf9fc]" : "border-[#e5e7eb] bg-[#f7f7f8]"}`}
                        >
                          <span className="block text-sm font-medium">{label}</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {options.skinTypes.map(([key, label]) => (
                        <button
                          ...
                          className={`text-left rounded-2xl border px-4 py-3 transition ${form.skinType === key ? "border-[#0ea5b7] bg-[#eaf9fc]" : "border-[#e5e7eb] bg-[#f7f7f8]"}`}
                        >
                          <span className="block text-sm font-medium">{label}</span>
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  {step < 4 ? (
                    <button
                      ...
                      className="rounded-xl bg-[#0ea5b7] text-white px-5 py-2 text-sm font-semibold"
            ...
            {step === 5 && (
              <div className="mt-7">
                <div className="rounded-2xl bg-[#f3fbfd] border border-[#d3eef3] p-4 mb-6">
                ...
                <p className="text-sm text-gray-500 mt-1">Includes exactly one mandatory cleanser, toner, serum, moisturizer, and oil.</p>
