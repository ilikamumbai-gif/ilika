    ...
    <div className="bg-[#FFF8F5] text-[#3A2A27] [font-family:'DM_Sans',sans-serif]">
        ...
        <div className="mx-auto grid min-h-[calc(100dvh-150px)] w-full max-w-[1240px] grid-cols-1 gap-6 lg:grid-cols-[1.45fr_0.95fr] lg:items-center lg:gap-10">
          <div className="flex w-full flex-col">
            ...
            <h1 className="mb-4 w-full max-w-[760px] [font-family:'Playfair_Display',serif] text-[clamp(34px,3.8vw,60px)] font-bold leading-[1.04] tracking-[-0.02em]">
              ...
              <span className="mt-1.5 block text-[0.72em] font-medium tracking-[0.02em] text-[rgba(58,42,39,0.62)]">Made by You. In Minutes.</span>
            </h1>
            ...
            <div className="mb-4 flex items-end gap-3">
              <span className="[font-family:'Playfair_Display',serif] text-[52px] font-bold leading-none">₹{discountedPrice.toLocaleString("en-IN")}</span>
            ...
            <button
              ...
              className="group relative mb-3 inline-flex h-[90px] w-full max-w-[760px] items-center justify-center overflow-hidden rounded-[14px] border border-[#8A4D40] bg-[linear-gradient(135deg,_#E36A4F_0%,_#C9553F_45%,_#B34838_100%)] px-[52px] py-5 text-[18px] font-bold tracking-[0.03em] text-white shadow-[0_14px_30px_rgba(179,72,56,0.35)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(179,72,56,0.45)]"
            >
            ...
            <div className="mb-3 flex w-full max-w-[760px] flex-wrap gap-3">
              <button
                ...
                className="inline-flex h-[48px] min-w-[320px] flex-1 items-center justify-center gap-2 rounded-[10px] border border-dashed border-[rgba(198,160,109,0.65)] bg-[rgba(198,160,109,0.14)] px-4 text-[12px] font-semibold tracking-[0.05em] text-[#B68544] transition hover:bg-[rgba(198,160,109,0.22)]"
              >
                <Tag className="h-3.5 w-3.5" />
                {couponApplied ? "Coupon Applied: ilikaDIY · 15% Off" : "Use Code: ilikaDIY · Flat 15% Off"}
                {!couponApplied && (
                  <span className="inline-flex animate-bounce items-center gap-1 rounded-full border border-[#E6B878] bg-[#FFF2DC] px-2 py-[2px] text-[10px] font-bold uppercase tracking-[0.08em] text-[#A86B27]">
              ...
              <a
                href={productPath}
                className="inline-flex h-[48px] min-w-[170px] items-center justify-center px-1 text-[15px] font-semibold text-[#8F5A4E] underline-offset-2 hover:text-[#75483F] hover:underline"
              >
                Know More ↓
              </a>
            </div>
            ...
            <div className="mt-1 flex items-center gap-2.5 text-[12px] text-[rgba(126,102,96,0.9)]">
              ...
              <span>
                Offer ends in <span className="font-semibold text-[#9E5C4F]">{countdown}</span> · Only 12 units left
              </span>
            </div>

            {loading && !targetProduct && (
              <p className="mt-2 text-[12px] text-[#8F5A4E]">Loading live product details...</p>
            )}
          </div>
          ...
          <div className="relative z-[3] aspect-[4/4] w-full border-4 border-white bg-white p-3 shadow-[0_14px_30px_rgba(125,80,66,0.14)]">
            <img
              src={productImage}
              alt={productName}
              className="h-full w-full bg-[#F7F7F7] object-contain object-center"
              onError={(e) => {
      ...
      <div className="flex flex-wrap justify-center gap-6 border-y border-[rgba(184,113,97,0.28)] bg-[linear-gradient(120deg,_#2F1F1C_0%,_#472E2A_100%)] px-[8%] py-5 lg:gap-12">
        {[
          ...
          <div key={label} className="flex items-center gap-2.5 text-[13px] font-medium text-[rgba(245,237,230,0.9)]">
            <span className="grid h-8 w-8 place-content-center rounded-full border border-[rgba(233,180,157,0.55)] bg-[rgba(233,180,157,0.2)] text-base">
      ...
      <section id="features" className="px-[8%] py-20">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {features.map((f) => (
            <div key={f.title} className="rounded-[20px] border border-[#EBDDD4] bg-[#FFFDFC] px-7 py-9 transition hover:-translate-y-1 hover:shadow-[0_14px_40px_rgba(184,113,97,0.14)]">
              <div className="mb-5 grid h-[52px] w-[52px] place-content-center rounded-[14px] bg-[#F6E7DE] text-[26px]">
                <f.icon className="mx-auto h-6.5 w-6.5 text-[#9E5C4F]" />
              </div>
              <h3 className="mb-2.5 text-[17px] font-semibold">{f.title}</h3>
              <p className="text-[14px] font-light leading-[1.65] text-[#7E6660]">{f.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={title} className="text-center">
              ...
              <p className="text-[13px] font-light leading-[1.6] text-[#7E6660]">{desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[linear-gradient(180deg,_#FFFDFB_0%,_#F9EFE7_100%)] px-[8%] py-20">
        <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2 lg:gap-[60px]">
          <div>
            ...
            <p className="mb-7 text-[15px] font-light leading-[1.7] text-[#7E6660]">
            ...
            <div className="flex flex-wrap gap-2.5">
              {["Orange", "Kiwi", "Strawberry", "Lemon", "Milk", "Honey", "Avocado", "Collagen"].map((tag) => (
                <span key={tag} className="rounded-full border border-[#EBCFC2] bg-[#F6E7DE] px-[18px] py-2 text-[13px] font-medium text-[#9E5C4F]">{tag}</span>
              ))}
            </div>
          </div>

          <div className="rounded-[28px] bg-[#F6E7DE] px-10 py-12">
            <div className="mb-4 rounded-2xl border border-[#EBCFC2] bg-[#FFFDFC] p-6">
              ...
              <p className="text-[13px] font-light text-[#7E6660]">
            ...
            <div className="rounded-2xl border border-[#EBCFC2] bg-[#FFFDFC] p-6">
              <p className="mb-2 inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#B87161]">
      ...
      <section className="bg-[linear-gradient(180deg,_#FFFFFF_0%,_#FFF5EE_100%)] px-[8%] py-20">
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
          {reviews.map((r) => (
            <div key={r.name} className="rounded-2xl border border-[#EBDDD4] bg-[#FFF9F6] px-6 py-7">
              <div className="mb-3 flex items-center gap-1 text-[#D3A157]">
                {[1, 2, 3, 4, 5].map((n) => (
                  <Star key={n} className={`h-4 w-4 ${n <= r.rating ? "fill-current" : "opacity-35"}`} />
                ))}
              </div>
              <p className="mb-5 text-[14px] italic font-light leading-[1.7] text-[#7E6660]">{r.text}</p>
              <div className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-content-center rounded-full bg-[#F1D7CD] text-[13px] font-semibold text-[#9E5C4F]">{r.initials}</span>
                <div>
                  <p className="text-[13px] font-semibold">{r.name}</p>
                  <p className="text-[12px] text-[#7E6660]">{r.loc}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(120deg,_#2F1F1C_0%,_#472E2A_100%)] px-[8%] py-20 text-center">
        ...
        <div className="relative z-[1]">
          <button
            type="button"
            onClick={handleBuyNow}
            className="mb-3 inline-block rounded-full bg-[#FFF7F2] px-12 py-[18px] text-[16px] font-semibold text-[#9E5C4F] transition hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(0,0,0,0.2)]"
          >
          ...
          <a
            href={productPath}
            className="mb-3 ml-2 inline-block rounded-full border-2 border-[rgba(255,255,255,0.7)] px-9 py-4 text-[16px] font-medium text-white transition hover:bg-[rgba(255,255,255,0.12)]"
          >
            Learn More ↗
          </a>
        </div>
