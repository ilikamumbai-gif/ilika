  ...
  return (
    <div className="bg-[#FFF8F5] text-[#3A2A27] [font-family:'DM_Sans',sans-serif]">
      ...
      <section className="cv-auto bg-[linear-gradient(180deg,_#FFFFFF_0%,_#FFF5EE_100%)] px-[8%] py-20">
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">{reviews.map((r) => (<div key={r.name} className="rounded-2xl border border-[#EBDDD4] bg-[#FFF9F6] px-6 py-7"><div className="mb-3 flex items-center justify-between gap-3"><div className="flex items-center gap-1 text-[#D3A157]">{[1, 2, 3, 4, 5].map((n) => (<Star key={n} className={`h-4 w-4 ${n <= r.rating ? "fill-current" : "opacity-35"}`} />))}</div><p className="text-[13px] font-semibold">{r.name}</p></div><p className="mb-5 text-[14px] italic font-light leading-[1.7] text-[#7E6660]">{r.text}</p><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-content-center rounded-full bg-[#F1D7CD] text-[13px] font-semibold text-[#9E5C4F]">{r.initials}</span><div><p className="text-[12px] text-[#7E6660]">{r.loc}</p></div></div></div>))}</div>
      </section>
