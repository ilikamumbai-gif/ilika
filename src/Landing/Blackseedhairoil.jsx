      ...
      <main className="min-h-screen overflow-x-hidden bg-[#FAFAF7] text-[#1a1a1a]">
        ...
        <section className="bg-[#F5F0E8] px-4 py-16 sm:px-6 lg:px-[5%] lg:py-24">
          <div className="mx-auto max-w-[1100px]">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{landingReviews.map((r, idx) => <div key={r.id || `${r.name}_${idx}`} className="rounded-[20px] border border-[#EAE4D8] bg-white px-7 pb-6 pt-7"><div className="flex items-center justify-between gap-3"><StarRating n={r.stars} /><div className="font-sans text-sm font-semibold">{r.name}</div></div>{r.image ? <img src={r.image} alt={`${r.name} review`} className="mt-4 h-40 w-full rounded-xl object-cover" /> : null}<p className="my-4 font-sans text-[14.5px] italic leading-7 text-[#3c3830]">"{r.text}"</p><div><div className="font-sans text-xs text-[#8a7f6e]">{r.loc} - Verified</div></div></div>)}</div>
          </div>
        </section>
        ...
        <section className="bg-[#1a1a1a] px-4 py-16 text-center text-white sm:px-6 lg:px-[5%] lg:py-24">
          ...
          <p className="mx-auto mb-10 mt-5 max-w-[520px] font-sans text-base leading-8 text-[#ccc] lg:text-lg">Join thousands of happy customers who've reclaimed their natural hair colour and vitality with Ilika Black Seed Hair Growth Oil.</p>
