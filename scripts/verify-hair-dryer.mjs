...
const landing = await fs.readFile("dist/leafless-hair-dryer-landing/index.html", "utf8");
assert.ok(landing.includes('data-prerendered="hair-dryer-landing"'));
assert.ok(landing.includes(`<link rel="canonical" href="${canonical}"`));
assert.ok(landing.includes("₹2,699 (MRP ₹5,999)"));
const routes = await fs.readFile("src/Routes/NavRoutes.jsx", "utf8");
assert.ok(routes.includes('<Route path="/leafless-hair-dryer-landing" element={renderLazy(HairDryerLanding)} />'));
const blogLinks = await fs.readFile("src/data/privateBlogs.js", "utf8");
assert.ok(blogLinks.includes('const hairDryerLandingPath = "/leafless-hair-dryer-landing";'));
