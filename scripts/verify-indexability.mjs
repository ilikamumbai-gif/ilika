...
async function main() {
  ...
  const journal = await readFile(path.join(distDir, "blog", "index.html"));
  if (!journal.includes('href="/articles"')) throw new Error("Blog page must link to the article directory in initial HTML.");
  const blogIndex = await readFile(path.join(distDir, "articles", "index.html"));
  ...
  for (const url of blogUrls) {
    if (!articleLinks.has(new URL(url).pathname) && !articleLinks.has(url)) {
      throw new Error(`Article directory initial HTML is missing article link: ${url}`);
    }
  }
  if (Array.from(articleLinks).some(href => href.startsWith("/blog/private/"))) {
    throw new Error("Article directory exposes private article links.");
  }
